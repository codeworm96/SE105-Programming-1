...
Regular_triangle::Regular_triangle(Point center, int dist)
{
	math::Vector2d vec(0, -dist);//the vector of the center to the vertex
	for (int i = 0; i < 3; ++i){//generate the three vertices
		add(Point(center.x + vec.x, center.y + vec.y));
		vec = vec.rotate(math::PI * 2 / 3);//next vector
	}
}
