...
Hexagon::Hexagon(Point center, int dist)
{
	math::Vector2d vec(dist, 0);//the vector of the center to the vertex
	for (int i = 0; i < 6; ++i){//generate the six vertices
		add(Point(center.x + vec.x, center.y + vec.y));
		vec = vec.rotate(math::PI / 3);//next vector
	}
}
