...
class Poly :public Closed_polyline //because there is a bug in Polygon, so inherit from Closed_polyline
{
public:
	Poly(const vector<Point>& vp);//constructor
private:
	bool crossed(Point a1, Point a2, Point b1, Point b2);//test if a1a2 crosses b1b2
	bool on_segment(Point a, Point p1, Point p2);//test if a on p1p2
};
